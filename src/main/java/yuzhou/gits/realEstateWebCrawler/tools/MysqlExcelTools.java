...
public class MysqlExcelTools {
	...
	public void loadCfg(String path) throws Exception {
		...
		try {
			...
			if ((newLine = buffReader.readLine()) != null) {//host,port,user,pwd
				...
				this.driverClzName = vals[0];
				this.url = vals[1];
				this.user = vals[2];
				this.pwd = vals[3];
			}else{
				throw new Exception("配置文件错误,没有对应的主机，用户名，密码");
			}
			newLine = null;
			if ((newLine = buffReader.readLine()) != null) {// 
				this.destDir = newLine;
			}else{
				throw new Exception("配置文件错误,没有对应的目标目录");// excel dest dir
			}
			newLine = null;
			if ((newLine = buffReader.readLine()) != null) {// 
				this.xlsxBaseFileName = newLine;
			}else{
				throw new Exception("配置文件错误,没有对应的目标EXCEL名称");// excel name
			}
			newLine = null;
	...
	public static void main(String[] args) {
		...
		try {
			MysqlExcelTools tool = new MysqlExcelTools();
			tool.loadCfg("e:\\宁波excel");
			tool.dataSrcFactory = new DatabaseSourceFactory();
			Object[] params = { tool.driverClzName,tool.url,tool.user,tool.pwd };
			tool.dataSrcFactory.init(params);
	...
	public void export(String dir, String xlsxBaseFileName) throws Exception {
		...
		for (int sheetIdx = 0; sheetIdx < this.sheetNames.length; sheetIdx++) {
			...
			this.dataSrcFactory.prepareSource(sheetCollection);
